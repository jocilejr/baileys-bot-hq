    ...
    >
      ...
      >
        {bars.map((h, i) => {
          ...
          const isPlayedBar = barPos < progress;
          return (
            <div
              key={i}
              ...
              style={{
                height: `${h * 14}px`,
                backgroundColor: isPlayedBar ? "hsl(var(--primary) / 0.7)" : "hsl(var(--muted-foreground) / 0.2)",
              }}
            />
          );
        })}
...
function StepRow({
  ...
  const renderPreview = () => {
    switch (d.type) {
      case "sendText": {
        ...
        return (
          <div className="mx-1 mt-1 px-2.5 py-2 rounded-lg bg-muted/60 border border-border/30">
            <p className="text-[11px] text-foreground/80 whitespace-pre-wrap line-clamp-3 leading-relaxed" dangerouslySetInnerHTML={{ __html: html }} />
          </div>
        );
      }
      case "sendImage": {
        ...
        return (
          <div className="mx-1 mt-1 space-y-1.5">
            <div className="w-full rounded-lg bg-black/20 border border-border/30 overflow-hidden flex items-center justify-center">
              <img src={d.mediaUrl} alt="Preview" className="w-full max-h-[140px] object-contain" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
            </div>
            {d.caption && <p className="text-[11px] text-muted-foreground truncate px-1">{d.caption}</p>}
          </div>
        );
      }
      case "sendVideo": {
        ...
        return (
          <div className="mx-1 mt-1 space-y-1">
            <div className="relative w-full h-[60px] rounded-lg bg-muted/80 border border-border/30 flex items-center justify-center overflow-hidden">
              <video src={d.mediaUrl} className="w-full h-full object-cover absolute inset-0" muted preload="metadata" />
            ...
            {d.caption && <p className="text-[10px] text-muted-foreground truncate px-1">{d.caption}</p>}
          </div>
        );
      }
      case "sendAudio": {
      ...
      case "waitForClick":
        return null;
      case "sendFile": {
        const fName = d.fileName || (d.fileUrl ? (d.fileUrl as string).split("/").pop() : null);
        return (
  ...
  if (d.type === "waitForClick") {
    const displayUrl = d.clickUrl
      ? ((d.clickUrl as string).length > 35 ? (d.clickUrl as string).substring(0, 35) + "..." : d.clickUrl)
      : "URL não definida";
    return (
      <div
        ref={containerRef}
  ...
  if (d.type === "sendText" && d.textContent) {
    ...
    return (
      ...
      >
        ...
        <p className="text-[13px] text-foreground/85 whitespace-pre-wrap line-clamp-8 leading-relaxed flex-1 min-w-0" dangerouslySetInnerHTML={{ __html: html }} />
      </div>
    );
  }
...
function GroupNode({ id, data, selected }: GroupNodeProps) {
  ...
  const accentColor = headerConfig?.color || "#22c55e";
  ...
  const handleDragEnd = useCallback((e: React.DragEvent) => {
    ...
    if (groupEl) {
      ...
      if (outside && dragIndex !== null) {
        ...
        if (step) {
          document.dispatchEvent(new CustomEvent("group-extract-step", {
    ...
    if (dragIndex !== null && overIndex !== null && dragIndex !== overIndex) {
      document.dispatchEvent(new CustomEvent("group-reorder-step", {
  ...
  return (
    <div className="relative group/card" style={{ background: "transparent" }}>
      <div
        className="absolute -top-10 left-1/2 -translate-x-1/2 z-50 flex items-center gap-0.5 bg-card/95 backdrop-blur-sm border border-border/60 rounded-lg shadow-[0_4px_20px_rgba(0,0,0,0.4)] p-1 opacity-0 group-hover/card:opacity-100 transition-opacity nopan nodrag"
        onMouseDown={(e) => e.stopPropagation()}
        onPointerDown={(e) => e.stopPropagation()}
      >
        <button
      ...
      <div
        className={`${hasFinalizerStep ? "w-[320px]" : "w-[280px]"} rounded-xl overflow-visible transition-all duration-200 bg-card border ${
          isDockTarget
            ...
            : "border-border/60 shadow-[0_4px_24px_rgba(0,0,0,0.35)] hover:shadow-[0_8px_32px_rgba(0,0,0,0.45)]"
        }`}
      >
        <div className="group-drag-handle flex items-center gap-2.5 px-3 py-2.5 border-b border-border/40 cursor-grab active:cursor-grabbing bg-secondary/30">
          <div
        ...
        <div
          ...
          onDrop={(e) => {
            ...
            if (stepId && sourceNodeId && sourceNodeId !== id) {
              e.preventDefault();
              e.stopPropagation();
              document.dispatchEvent(new CustomEvent("group-receive-step", {
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          ...
                          document.dispatchEvent(new CustomEvent("group-add-step", {
